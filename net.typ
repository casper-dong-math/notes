...
#title[
  Universal Nets in Functional Analysis
]
...
*Definition.* A net $(x_alpha )$ in a set $X$ is called a *universal net* if, for every subset $E subset X$, the net is either eventually in $E$ or eventually in $X - E$.
...
*Property 1 (Existence):* Every net in $X$ has a universal subnet. (We omit the proof, but note its equivalence to the existence of maximal ideals).

_Proof._ omitted. The statement is equivalent to the Axiom of Choice. 

*Property 2 (Image is Universal):* If $(x_alpha )$ is a universal net in $X$ and $f : X -> Y$ is any map, then $(f (x_alpha ))$ is a universal net in $Y$.

_Proof._ Consider preimage of $U$ and $Y - U$ in $X$. $(x_alpha )$ is eventually in either preimages, therefore $(f (x_alpha ))$ is eventually in either $U$ or $Y-U$.

*Property 3 (Converge to Cluster Points):* If a universal net $(x_alpha )$ has a cluster point $x in X$, it must converge to $x$.

_Proof._ If the net frequently visits a neighborhood $U$ of $x$, it cannot be eventually in $X - U$. By universality, it must therefore be eventually in $U$.

*Property 4 (Compactness):* A topological space $X$ is compact if and only if every universal net in $X$ converges.

_Proof._ $X$ compact $<=>$ Every net in $X$ has a cluster point $<=>$ Every universal net in $X$ converges by existence of universal subnet.

*Corollary 5 (Relative Compactness):* A subspace $Y$ of $X$ is relatively compact if and only if every universal net in $Y$ converges in $X$.
...
== 0. Tychonoff's Theorem
*Theorem.* Product $X = Pi_(i in I) X_i$ of compact topologies $(X_i)_(i in I)$ is compact.
...
== 1. The Banach-Alaoglu Theorem
*Theorem.* Let $V$ be a normed vector space. The closed unit ball $overline(BB)_(V^*)$ of the topological dual space $V^*$ is compact in the weak-\* topology.

*Proof.* Let $(f_alpha )$ be a net in $overline(BB)_(V^*)$. Pass to a universal subnet $(f_beta )$. For any vector $v in V$, the scalar net $(f_beta (v))$ is a universal net in the compact disk $norm(v) overline(BB)$. Then $(f_beta (v))$ converges uniquely to, say, $c (v)$.
...
*Theorem.* Let $X$ be an LCH and $Y$ metrizable. If a family of continuous maps $F subset C (X, Y)$ satisfies
...
*Proof.* Let $(f_alpha )$ be a universal net in $F$. For each $x in X$, the net $f_alpha (x)$ is a universal net in its relatively compact image, and therefore converges uniquely to some limit $f (x)$ in $Y$. This gives us a pointwise limit $f : X -> Y$.
...
Furthermore, on any compact set $K subset X$, equicontinuity along with the pointwise convergence of the universal net upgrades the convergence to uniform convergence via a standard finite subcover argument (check!). Thus $f_alpha -> f$ uniformly on compact sets i.e. in the compact-open topology. $square$

== 3. Generalized Helly's Selection Theorem

*Theorem.* Let $X$ be a partially ordered set, and $Y$ a compact space equipped with a closed partial order. Then a net of increasing (resp. decreasing) functions $f_alpha : X -> Y$ possesses a pointwise convergent subnet whose limit is increasing (resp. decreasing).
